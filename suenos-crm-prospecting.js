    ...
    try {
      const [{ data: m, error: e1 }, { data: enr, error: e2 }] = await Promise.all([
        sb.from('outreach_messages')
          .select('id,email,status,queued_at,sent_at,consent_evidence,licenses(establishment,city)')
      ...
      {!msgs.length ? (
        ...
        return (
          <Card key={m.id} cls="p-3">
            ...
            {open && (
              <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-800 text-[10px] text-gray-500 dark:text-gray-400 space-y-1">
