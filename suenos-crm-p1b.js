// PART 1-B: Core (no Recharts import)
...
const INIT_PRODUCTS = [
  { id:'p1', name:'Suenos Blanco',     sku:'SB-750', size:'750ml', casePack:12, price:60, active:true },
  { id:'p2', name:'Suenos Reposado',   sku:'SR-750', size:'750ml', casePack:12, price:70, active:true },
  { id:'p3', name:'Suenos Anejo',      sku:'SA-750', size:'750ml', casePack:12, price:80, active:true },
  { id:'p4', name:'Suenos Cristalino', sku:'SC-750', size:'750ml', casePack:12, price:90, active:true },
];
...
const INIT_ACCOUNTS = [
  { id:'a1',  name:'Botanist Restaurant',         type:'Restaurant',           region:'Vancouver',         assignedRep:'u2', address:'1495 W Georgia St, Vancouver, BC',      lat:49.2899, lng:-123.1252, contact:'Marcus Webb',      email:'marcus@botanistvan.com',        phone:'604-682-8383', website:'botanistrestaurant.com', status:'Active',        notes:'Hotel restaurant. Strong Anejo cocktail program.',          lastVisit:'2026-05-28', lastOrder:'2026-05-15', menuPlacements:{Margarita:'Permanent Placement',Paloma:'Seasonal Feature'},                           createdAt:'2025-10-01' },
  { id:'a2',  name:'Nightingale Restaurant',      type:'Restaurant',           region:'Vancouver',         assignedRep:'u2', address:'1017 W Hastings St, Vancouver, BC',     lat:49.2866, lng:-123.1202, contact:'Sophie Chen',      email:'sophie@hawksworth.ca',          phone:'604-695-9115', website:'nightingalevan.com',     status:'Active',        notes:'Hawksworth property. Seasonal menus. Strong cocktails.',    lastVisit:'2026-05-30', lastOrder:'2026-05-20', menuPlacements:{Margarita:'Permanent Placement','Ranch Water':'Discussing'},                          createdAt:'2025-09-15' },
  ...
  { id:'a13', name:'Mission Hill Winery Estate', type:'Resort',               region:'Kelowna / Okanagan',assignedRep:'u2', address:'1730 Mission Hill Rd, West Kelowna, BC', lat:49.8728, lng:-119.6087, contact:'Claire Fontaine',  email:'claire@missionhillwinery.com',  phone:'250-768-7611', website:'missionhillwinery.com',  status:'Prospect',      notes:'Premium estate winery. High-end clientele.',               lastVisit:'2026-03-10', lastOrder:null,         menuPlacements:{},                                                                                     createdAt:'2026-01-01' },
  { id:'a14', name:'BC Liquor Store Granville',  type:'Government Liquor Store',region:'Vancouver',       assignedRep:'u2', address:'1155 Granville St, Vancouver, BC',      lat:49.2793, lng:-123.1261, contact:'BCLDB Area Manager',email:'bcldb@gov.bc.ca',              phone:'604-660-0704', website:'bcl.ca',                 status:'Listed',        notes:'Government store. Standard BCLDB ordering process.',        lastVisit:'2026-04-20', lastOrder:'2026-04-01', menuPlacements:{},                                                                                     createdAt:'2025-05-01' },
...
const INIT_VISITS = [
  ...
  { id:'v3',  accountId:'a8',  date:'2026-05-29', contact:'Ryan OBrien',      type:'Staff Training',   notes:'Trained 6 staff on cocktail recipes. Blanco and Reposado featured.',       outcome:'Great reception. Ranch Water confirmed as permanent on menu.', followUpDate:'2026-06-20', repId:'u3', checks:{shelf:false,menu:true, display:false,samples:true, sellSheet:false} },
  { id:'v4',  accountId:'a9',  date:'2026-06-02', contact:'Michelle Tan',     type:'Follow Up',        notes:'Quick check-in. Sales going well. Michelle wants to trial Anejo.',         outcome:'Sending Anejo sample. Follow up in 1 week.', followUpDate:'2026-06-09', repId:'u3', checks:{shelf:false,menu:true, display:false,samples:true, sellSheet:false} },
  ...
  { id:'v10', accountId:'a3',  date:'2026-05-10', contact:'Thomas Lee',       type:'Product Sampling', notes:'Presented Anejo and Cristalino. Thomas keen on Cristalino upgrade.',       outcome:'Pending F&B director approval.', followUpDate:'2026-05-31', repId:'u2', checks:{shelf:false,menu:false,display:false,samples:true, sellSheet:true}  },
];
...
const INIT_ORDERS = [
  ...
  { id:'o7', accountId:'a1',  productId:'p3', bottles:6,  requestedDate:'2026-06-08', storeId:'fs1', status:'Submitted', notes:'Anejo for cocktail menu',   repId:'u2', createdAt:'2026-05-28' },
...
const INIT_TASKS = [
  ...
  { id:'t7', accountId:'a9',  title:'Deliver Anejo sample',                     dueDate:'2026-06-09', repId:'u3', done:false, priority:'medium' },
...
const INIT_TASTINGS = [
  ...
  { id:'ta3', accountId:'a12', date:'2026-05-30', location:'Village Wines - Weekend Tasting',      staff:'Aria Vega',    bottlesUsed:2, samplesServed:38, notes:'Saturday afternoon. Anejo standout. Same-day reorder.' },
];
...
function healthInfo(score) {
  if (score>=70) return { label:'Healthy',    color:'text-emerald-600 dark:text-emerald-400', bg:'bg-emerald-50 dark:bg-emerald-900/20', dot:'bg-emerald-500', ring:'ring-emerald-300 dark:ring-emerald-700' };
  if (score>=40) return { label:'Watch List', color:'text-amber-600 dark:text-amber-400',   bg:'bg-amber-50 dark:bg-amber-900/20',   dot:'bg-amber-500',   ring:'ring-amber-300 dark:ring-amber-700'   };
  return              { label:'At Risk',    color:'text-red-600 dark:text-red-400',       bg:'bg-red-50 dark:bg-red-900/20',       dot:'bg-red-500',     ring:'ring-red-300 dark:ring-red-700'       };
}
...
const INITIAL_STATE = {
  user:null, view:'dashboard', params:{}, darkMode:false, sidebarOpen:true,
...
function Btn({ children, variant='primary', size='md', onClick, type='button', disabled, cls='' }) {
  const base = 'inline-flex items-center gap-1.5 font-medium rounded-lg transition-all focus:outline-none disabled:opacity-40 disabled:cursor-not-allowed';
  const sz = { sm:'px-2.5 py-1.5 text-xs', md:'px-3.5 py-2 text-sm', lg:'px-5 py-2.5 text-sm' };
  const vr = {
...
function Card({ children, cls='', onClick }) {
  return <div onClick={onClick} className={`bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm ${onClick?'cursor-pointer hover:shadow-md transition-shadow':''} ${cls}`}>{children}</div>;
}
  ...
  return (
    <Card cls="p-4 sm:p-5">
      <div className="flex items-start justify-between">
        ...
        <div className={`ml-3 p-2.5 rounded-xl flex-shrink-0 ${cc[color]||cc.amber}`}>
...
function EmptyState({ icon='info', title, desc, action }) {
  return (
    <div className="text-center py-12 px-4">
      <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-400 mb-3">
        <Ic n={icon} cls="w-6 h-6"/>
      </div>
...
function ProgressBar({ value, max, color='amber' }) {
  ...
  const cc = { amber:'bg-amber-500', emerald:'bg-emerald-500', blue:'bg-blue-500', rose:'bg-rose-500' };
  return (
...
// Aliases for backward-compat with part3
const Input  = FInput;
